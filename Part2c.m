...
%randomized sampling to produce the SVD (inspired by a post on the matlab forums)
[m,n] = size(D);
K=input('\n Enter the desired number of K projections ');
P = min(2*K,n);
X = randn(n,P);
Y = D*X;
W1 = orth(Y);
...
%taking the svd od d for comparison
[u, s, v] = svd(D);

%plots to compare the approximate modes vs the true modes of the matrix D
plot(U)
plot(u)
plot(S)
plot(s)
plot(V) %for posterity
plot(v)
